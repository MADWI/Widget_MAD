...
public class PlanChangesActivity extends Activity{
	...
	private class AsyncTaskGetPlanChanges extends AsyncTask <Context, Boolean, Void>{
		...
		protected void onProgressUpdate(Boolean... values) {
			...
			if (values[0]==false) {
				showToast("Nie mo¿na pobraæ wiadomoœci: (",3000,ctx);
			} else if (values[0]==true ) {
				showToast("Zakoñczono !",3000,ctx);
			}
		...
		@Override
		 protected void onPreExecute() 
		 {
			...
			pd = ProgressDialog.show(PlanChangesActivity.this,"Odsiwe¿anie wiadomoœci","Proszê czekaæ...",true,false,null);
